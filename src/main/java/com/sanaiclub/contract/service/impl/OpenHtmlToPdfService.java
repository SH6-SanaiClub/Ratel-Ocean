...
/** 계약서 PDF 생성 서비스 구현체. OpenHTMLToPDF 사용. HTML 템플릿 생성 후 PDF 변환. */
@Service
public class OpenHtmlToPdfService implements ContractPdfService {

    private static final Logger logger = LoggerFactory.getLogger(OpenHtmlToPdfService.class);

    /** 계약서 PDF 생성. HTML 템플릿 생성 후 OpenHTMLToPDF로 PDF 변환. 한글 폰트(맑은 고딕) 지원. */
    @Override
